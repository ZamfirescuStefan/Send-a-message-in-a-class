...
class Graph:  # graful problemei
    ...
    def calculeaza_euristica(self, x, y):
        ...
        if self.tip_euristica == 1:  # manhattan distance
            ...
            rand_nod_final, coloana_nod_final = self.find_elev(self.receptor)
            return sqrt((x - rand_nod_final) ** 2 + (y - coloana_nod_final) ** 2)
        else:
    ...
    def addToSucc(self, nodCurent, vecin, elevi_inaccesibili, info_prof, listaSuccesori, afis_chr):
        ...
        infomationNode = [(x, y), info_prof, vecin, afis_chr]
        if self.sunt_certati(student_actual, vecin) is False and vecin not in elevi_inaccesibili and \
                not nodCurent.contineInDrum(infomationNode[:-1]):
            cost = nodCurent.g + 1
            h = self.calculeaza_euristica(x, y)
            nodNou = NodParcurgere(infomationNode, nodCurent, cost, h)
...
def a_star(gr, nrSolutiiCautate, timeout):
    ...
    while len(c) > 0:
        nodCurent = c.pop(0)
        # fout.write("Nodul {} are urmatorul drum: \n".format(nodCurent.info[0]))
        # nodCurent.afisDrum()
        # fout.write("\n")
        ...
        if gr.testeaza_scop(nodCurent):
            ...
            nodCurent.afisDrum()
            fout.write("\nTimpul de gasire al solutiei: {}".format(actual_time - start_time))
            fout.write("\n----------------\n\n")
            nrSolutiiCautate -= 1
            index_sol += 1
        ...
        # Debug
        # fout.write("nodul {} are urmatorii succesori: \n".format(nodCurent))
        # for elem in lSuccesori:
        #     elem_sir = str(elem.info)
        #     fout.write(elem_sir)
        # fout.write("\n")
        # End debug
...
def ucs(gr, nrSolutiiCautate, timeout):
    ...
    while len(c) > 0:
        ...
        if gr.testeaza_scop(nodCurent):
            ...
            fout.write(f"Solutie {index_sol}: \n")
            nodCurent.afisDrum()
            fout.write("\nTimpul de gesire al solutiei: {}".format(acutual_time - start_time))
            fout.write("\n----------------\n\n")
            nrSolutiiCautate -= 1
            index_sol += 1
            if nrSolutiiCautate == 0:
...
def make_files(dir_in, dir_out):
    try:
        ...
        print("Path to test1 file is invalid, exiting...")
        quit()
...
def main():
    ...
    for index in range(len(input_files)):
        ...
        gr = Graph(lista_elevi, elevi_suparati, M, elevi_ascultati, emitator, receptor, tip_euristica)
